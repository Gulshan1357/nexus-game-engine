...
void InputManager::AddInputKeyToAction(const Input::PlayerID playerId, const int keyCode, const Input::PlayerAction action)
{
	if (playerId == Input::PlayerID::PLAYER_1)
	{

		m_playerOneKeyToActionMap[keyCode] = action;
	...
	else if (playerId == Input::PlayerID::PLAYER_2)
	{
		...
		m_playerTwoKeyToActionMap[keyCode] = action;
